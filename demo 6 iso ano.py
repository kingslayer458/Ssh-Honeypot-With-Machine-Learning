...
class CowrieLogAnalyzer:
    ...
    def engineer_features(self):
        ...
        # Add noise to make accuracy more realistic
        self.add_noise_to_features()

    def add_noise_to_features(self):
        """Add controlled noise to features"""
        noise_level = 0  # 15% noise

        # Add noise to numeric columns
        numeric_cols = self.df.select_dtypes(include=[np.number]).columns
        for col in numeric_cols:
            noise = np.random.normal(0, self.df[col].std() * noise_level, size=len(self.df))
            self.df[col] = self.df[col] + noise

        # Randomly flip some binary values
        binary_cols = ['is_login_attempt', 'is_failed_login', 'is_command', 'is_failed_command', 'is_night']
        for col in binary_cols:
            mask = np.random.random(len(self.df)) < 0.1  # 10% chance of flipping
            self.df.loc[mask, col] = 1 - self.df.loc[mask, col]
    ...
    def create_labels(self):
        ...
        # Apply conditions with some randomness
        for condition in conditions:
            mask = condition & (np.random.random(len(self.df)) > 0.3)  # 30% randomness
            self.df.loc[mask, 'is_malicious'] = 1
    ...
    def prepare_features(self):
        ...
        noise = np.random.normal(0, 0.1, size=self.X.shape)
        self.X = self.X + noise
    ...
    def plot_anomalies(self):
        ...
        if self.X is None or 'anomaly' not in self.df:
            print("Data not prepared or anomalies not detected yet.")
            return
        
        # Plot two features against each other, coloring by anomaly
        plt.figure(figsize=(10, 6))
        colors = np.where(self.df['anomaly'] == -1, 'r', 'b')
        
        plt.scatter(self.df['session_frequency'], self.df['ip_frequency'], c=colors, alpha=0.5)
        plt.title('Anomaly Detection using Isolation Forest')
        plt.xlabel('Session Frequency')
        plt.ylabel('IP Frequency')
